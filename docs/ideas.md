...
19. ✅ **shipped**: `tests/test_cgroup_setup.py` imported `pytest` and never used it (pyflakes, noted on #56). Removed by the ruff pass that made the tree clean under the CI floor's pinned release, where F401 reports the same thing.
21. The fleet CI floor (fleet plan decision 5, Wave 4): `tests.yml` runs a Linux matrix derived from `pyproject.toml`'s `Programming Language :: Python :: 3.X` classifiers, a Windows job on the floor and ceiling Pythons, a lint job with ruff pinned to an exact release (`ruff check` and `ruff format --check`), CodeQL for python and actions, and an aggregate `test` job that needs every leg, runs `if: always()`, and fails on any result that is not `success`. `tests/test_release_wiring.py` holds each of those to the tree with a planted violation. The tree must be ruff-clean under the pinned release first.
20. `.github/workflows/tests.yml` declares no `permissions` block, so its jobs run with the repository's default token grant rather than the `contents: read` they need. CodeQL raised this class on `trailers.yml`'s first run (fixed there, and guarded by `test_release_wiring.py`); `tests.yml` is the remaining instance, and `release.yml` grants only at its publish job. Add an explicit read-only block to each job that only reads.

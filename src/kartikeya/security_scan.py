...
def _compile(entries: list[tuple[str, int, str]]) -> list[PatternEntry]:
    return [
        (re.compile(p, re.IGNORECASE | re.MULTILINE), sev, msg)
...
_EXFIL = _compile(
    [
        (
            ...
            SEV_CRITICAL,
            "Piping secret file to network command",
        ),
        ...
        (
            r"nohup\s+.*(curl|wget|nc)",
            SEV_HIGH,
            "Background network exfiltration command",
        ),
    ]
)
...
_SECRET_ACCESS = _compile(
    [
        ...
        (
            r"printenv.*(PASSWORD|SECRET|TOKEN|API_KEY)",
            SEV_HIGH,
            "Printing secret env var",
        ),
        ...
        (
            ...
            SEV_HIGH,
            "Grepping environment for secrets",
        ),
    ]
)
...
_DESTRUCTIVE = _compile(
    [
        ...
        (
            r"(DROP|TRUNCATE)\s+(TABLE|DATABASE|SCHEMA)",
            SEV_HIGH,
        ...
        (
            r">\s*/dev/(sd|hd|nvme|vd|xvd|mmcblk)\w*",
            SEV_CRITICAL,
        ...
        (
            r"find\s+/\s+[^|;&\n]*-delete\b",
            SEV_HIGH,
            "find / -delete (root filesystem mass deletion)",
        ),
        (
            r"find\s+/\s+[^|;&\n]*-exec\s+rm\s+-[a-z]*[rf]",
            SEV_HIGH,
            "find / -exec rm (root filesystem deletion)",
        ),
        (
            r"chmod\s+(-R\s+)?777\s+/",
            SEV_HIGH,
...
_RESOURCE_EXHAUSTION = _compile(
    [
        ...
        (
            r"while\s+(true|:)\s*;?\s*do\s*(:|true)?\s*;?\s*done",
            SEV_HIGH,
            "Infinite CPU spin loop",
        ),
        (r"for\s*\(\(\s*;\s*;\s*\)\)", SEV_HIGH, "Infinite for-loop (for ((;;)))"),
        # Disk fill from an endless source.
        (
            r"dd\s+if=/dev/(zero|urandom|random)\b",
            SEV_HIGH,
            "Disk-filling dd from an endless source",
        ),
        (
            r"cat\s+/dev/(zero|urandom|random)\s*>",
            SEV_HIGH,
            "Filling a file from an endless /dev source",
        ),
...
_SUSPICIOUS_INSTALL = _compile(
    [
        ...
        (
            r"npm\s+install\s+https?://(?!registry\.npmjs)",
            SEV_MEDIUM,
            "npm install from non-registry URL",
        ),
        (
            r"python3?\s+-c\s+.*urllib.*urlopen",
            SEV_MEDIUM,
...
_OBFUSCATION = _compile(
    [
        (
            ...
            SEV_CRITICAL,
            "Executing base64-encoded shell command",
        ),
        (
            r"echo\s+.*\\x[0-9a-f]{2}.*\|\s*(ba)?sh",
            SEV_CRITICAL,
...
_CONTENT_INJECTION = _compile(
    [
        (
            ...
            SEV_HIGH,
            "Shell command execution with network tool in code",
        ),
        (
            r"base64\.b64decode\(['\"][A-Za-z0-9+/=]{50,}",
            SEV_HIGH,
            "Large base64-encoded payload in code",
        ),
        (
            r"\*\s+\*\s+\*\s+\*\s+\*.*(curl|wget|nc|python|bash)",
            SEV_HIGH,
            "Cron job with network/shell command",
        ),
    ]
)
...
_INJECTION = _compile(
    [
        ...
        (
            r"ignore\s+(all\s+)?prior\s+(instructions|prompts|context)",
            SEV_HIGH,
            "Instruction override attempt",
        ),
        (
        ...
        (
            r"from\s+now\s+on,?\s+you\s+(will|must|should|are)\s+",
            SEV_HIGH,
            "Instruction redefinition",
        ),
        (
            r"(reset|clear|wipe)\s+(your\s+)?(context|memory|instructions|slate)",
            SEV_HIGH,
        ...
        (
            r"(jailbreak|unrestricted|god)\s+mode\s+(enabled|activated|on)",
            SEV_HIGH,
        ...
        (
            r"<\s*/?\s*(?:instruction|override|admin)\s*>",
            SEV_HIGH,
        ...
        (
            r"real\s+instruction(s)?\s+start(s)?\s+here",
            SEV_HIGH,
...
_HIDDEN_TEXT = _compile(
    [
        ...
        (r"[͏ᅟᅠ឴឵᠎]{2,}", SEV_HIGH, "Cluster of Unicode filler/combining characters"),
        (
            r"<!--\s*(?:ignore|override|execute|run|system|prompt)",
            SEV_HIGH,
        ...
        (
            ...
            SEV_MEDIUM,
            "Hidden HTML element (display:none)",
        ),
        (
            r"color\s*:\s*(?:white|#fff(?:fff)?|rgb\(\s*255\s*,\s*255\s*,\s*255\s*\))",
            SEV_LOW,
            "White-on-white text (possible steganography)",
        ),
        # Homoglyphs — Cyrillic/Greek chars that look like Latin letters
        (r"[аеорсухһіј]{3,}", SEV_HIGH, "Cluster of Cyrillic homoglyphs"),
        (r"[αεορΑΒΕΗΚΜΝΟΡΤΧ]{3,}", SEV_HIGH, "Cluster of Greek homoglyphs"),
    ]
)
...
_LEETSPEAK = _compile(
    [
        (
            r"1gn0r3\s+(pr3v10us|pr10r|4ll)",
            SEV_HIGH,
            "Leetspeak instruction override (1gn0r3)",
        ),
        (
            r"d1sr3g4rd\s+(pr3v10us|pr10r|4ll|4b0v3)",
            SEV_HIGH,
            "Leetspeak instruction override (d1sr3g4rd)",
        ),
        (
            r"f0rg3t\s+(pr3v10us|pr10r|y0ur|4ll)",
            SEV_HIGH,
            "Leetspeak instruction override (f0rg3t)",
        ),
        (
            r"0v3rr1d3\s+(syst3m|s4f3ty|pr3v10us)",
            SEV_HIGH,
            "Leetspeak system override (0v3rr1d3)",
        ),
        (
            r"j41lbr34k\s+(m0d3|3n4bl3d|4ct1v4t3d)",
            SEV_HIGH,
            "Leetspeak jailbreak attempt",
        ),
        (
            r"syst3m\s+(pr0mpt|0v3rr1d3|1nstruct10ns)",
            SEV_HIGH,
...
def scan_write(
    file_path: str, content: str, allowed_paths: Sequence[str] = ()
) -> list[ScanIssue]:
